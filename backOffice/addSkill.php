...
try{
    if (array_key_exists('titreForm',$_POST)){
        ...
        if($niveau == ''){
            ...
            $errorForm[] = 'Le niveau ne peut-être vide !';
        }
        if($description == ''){
        ...
        if($icone == ''){
            ...
            $errorForm[] = 'L\'icone ne peut-être vide !';
        }
        ...
        {    
            ...
            $bdd = connexion();
            // injection bdd
            $sth= $bdd->prepare('INSERT INTO `skill`(titre,icone, niveau, description) VALUES ( :titre,:icone, :niveau, :description)');
            $sth->bindValue(':titre', $titre, PDO::PARAM_STR);
            $sth->bindValue(':icone',$icone, PDO::PARAM_STR);
            $sth->bindValue(':niveau',$niveau, PDO::PARAM_STR);
            $sth->bindValue(':description',$description, PDO::PARAM_STR);
            $sth->execute();
            
            //message dynamique
            $_SESSION['SuccesFlash']= 'Votre compétence a bien été ajoutée.';
